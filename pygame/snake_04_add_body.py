...
class MainWindow():
    ...
    def __creat_snake(self, snake_size = 25,speed = 2):
        if not self.snake_list :
            self.head = Head(self.screen, [0,0,snake_size,snake_size],speed)
            self.snake_list.append(self.head)
        elif self.snake_list:
            # print(self.snake_list[-1].rect)
            body_x = self.snake_list[-1].rect[0] - snake_size
            body_y = self.snake_list[-1].rect[1] 
            self.snake_body = Snake(self.screen,[body_x, body_y, snake_size,snake_size]) 
       ...
       # print(len(self.snake_list))
        for e, rect in enumerate(self.snake_list):
            if e == 0:
                ...
                x = self.snake_list[e-1].rect[0] - snake_size
                y = self.snake_list[e-1].rect[1]
                rect.update(self.direction,x,y)
...
class Snake():
    ...
    def update(self,direction,x=0,y=0):
        print(x,y)
        self.rect = x,y,25,25
